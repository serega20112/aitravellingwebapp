...
class GeocodingService:
    ...
    ВАЖНО: согласно политике Nominatim требуется корректный User-Agent и желательно email.
    Настраивается через конфиг: NOMINATIM_BASE_URL, NOMINATIM_USER_AGENT, NOMINATIM_EMAIL.
    """

    def __init__(self, base_url: Optional[str] = None, user_agent: Optional[str] = None, email: Optional[str] = None, logger: logging.Logger | None = None):
        self.base_url = base_url or "https://nominatim.openstreetmap.org"
        self.user_agent = user_agent or "aitravel-app/1.0"
        self.email = email
        self._logger = logger or logging.getLogger(__name__)

    def reverse_geocode(self, latitude: float, longitude: float, lang: str = "ru") -> Dict[str, Any]:
        """
        Выполняет реверс‑геокодинг и возвращает словарь с полями:
        {
    ...
    def search(self, query: str, lang: str = "ru", limit: int = 1) -> Dict[str, Any]:
        """
        Прямой геокодинг: текстовый запрос -> список совпадений. Возвращает первый результат:
        {
            "display_name": str | None,
            "lat": float | None,
            "lon": float | None,
        ...
        try:
            ...
            if not first:
                return {"display_name": None, "lat": None, "lon": None, "raw": {"results": []}}
